...
class SyncPersonnelReportJob implements ShouldQueue
{
        ...
        try {
            ...
            foreach ($csvLines as $line) {
                ...
                $personnelToUpsert[] = [
                    ...
                    'psr_code' => $data['PERSON_NUMBER'],
                    'email' => $data['EMAIL'] ?? null
                ];

                if(count($personnelToUpsert) === $batch)
                {
                    Personnel::upsert(
                        $personnelToUpsert,
                        ['psr_code'], // The column to check for uniqueness
                        ['name', 'email', 'updated_at'] // The columns to update if a record exists
                    );

                    Log::info("Successfully synced {" . count($personnelToUpsert) . "} records from {" . $this->reportPath . "}");
                    $personnelToUpsert = [];
                }
            }

            if (!empty($personnelToUpsert)) {
