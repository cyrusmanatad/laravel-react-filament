...
class SyncBranchPlantReportJob implements ShouldQueue
{
    ...
    public function handle(OracleBIService $oracleBiService): void
    {
        try {
            ...
            foreach ($csvLines as $line) {
                ...
                $plantToUpsert[] = [
                    'branch_code' => $data['ORGANIZATION_CODE'] ?? null,
                    'branch_desc' => $data['NAME'] ?? null
                ];

                if(count($plantToUpsert) === $batch)
                {
                    BranchPlant::upsert(
                        $plantToUpsert,
                        ['branch_code'], // The column to check for uniqueness
                        ['branch_desc', 'updated_at'] // The columns to update if a record exists
                    );

                    Log::info("Successfully synced {" . count($plantToUpsert) . "} records from {" . $this->reportPath . "}");
                    $plantToUpsert = [];
                }
            }

            if (!empty($plantToUpsert)) {
